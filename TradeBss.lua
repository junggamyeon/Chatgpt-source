...
local function altLoop()
    ...
    while anchor.Parent do
        local btn = getAcceptButton(anchor)
        if btn and shouldAccept(btn) then
            print("ALT: Trade Accept")
            smartClick(btn)
        end
        task.wait(0.5)
    end

    print("ALT: Trade done -> writefile")
    pcall(function()
